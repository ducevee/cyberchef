...
try {
    ...
    if ($stmt_avaliacoes->rowCount() > 0) {  
        while ($row_avaliacao = $stmt_avaliacoes->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'><strong>Avaliação feita por:</strong> $nome_usuario</p>";
            echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'><strong>Data:</strong> $created</p>";
            echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'><strong>Estrelas:</strong>";
            for ($i = 1; $i <= 5; $i++) {
                if ($i <= $qtde_estrelas) {
            ...
            echo "</p>";
            if (!empty($mensagem)){
                echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'><strong>Comentário:</strong> $mensagem</p>";
            }
            echo "</div>";
            if (isset($_SESSION['usuario_id']) && $_SESSION['usuario_id'] == $id_usuario) {
                ...
                echo "<button type='submit' style='padding: 10px 15px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; margin-right: 5px; background-color: #dc3545; color: white; font-family: \"Maven Pro\", sans-serif;'>Excluir Avaliação</button>";
                echo "</form>";
            }
            echo "<br><hr>";
        }
        ...
        echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'><strong>Média das avaliações:</strong> $mediaFormatada</p>";
    } else {
        echo "<p style='font-size: 16px; margin-bottom: 10px; font-family: \"Maven Pro\", sans-serif;'>Não há avaliações para esta receita ainda.</p>"; 
    }
